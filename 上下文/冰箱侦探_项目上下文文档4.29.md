...
| 继续推演公共技术地基 Phase 2 后，确认数据库与 services 设计前需要先解决“未注册用户身份”问题。建议采用 Supabase Anonymous Auth 承接首次零摩擦体验：用户首次打开 App 自动创建 anonymous user，获得 auth.users.id，所有私有表仍使用 user_id 归属；用户完成第一次做饭后再绑定邮箱、手机或 OAuth。这样可避免设计 guest_id 或 user_id nullable，并保持 RLS、Storage、services 边界清晰。                                                                                                                                                                                                                |
| 已确认采用 Supabase Anonymous Auth 作为未注册用户身份方案。进一步确认公共技术地基阶段可以按“受控纵向切片”推进：先以 User Profile / Onboarding Foundation 模块为第一切片，同时设计数据库表、types 和 services 边界。该模块包括 profiles、user_preferences、kitchen_equipment、pantry_items，对应 profileService.ts，并建议新增 authService.ts 负责 anonymous auth 初始化。                                                                                                                                                                                                                         |
| 已进一步确认 User Profile / Onboarding Foundation 模块的数据设计原则：user_preferences、kitchen_equipment、pantry_items 的核心价值是作为推荐算法输入，而不是泛化用户画像。偏好、设备、调料字段必须标准化、枚举化、key 化，避免自由文本导致推荐匹配不可控。确认 authService.ensureAuthUser() 独立于 profileService，用于承接 Anonymous Auth 初始化。                                                                                                                                                                                                                                               |
| 已完成 User Profile Module v0.1 Key Dictionary 设计。确认所有会进入推荐算法的用户输入均应标准化、枚举化、key 化，避免自由文本。当前确定的 key 体系包括：portion_size、spice_level、saltiness、cuisine_preferences、meal_style_preferences、diet_tags、disliked_ingredient_keys、allergen_keys、kitchen_equipment.equipment_key、pantry_items.pantry_item_key 和 pantry 分类。确认前端显示 label，数据库、菜谱库和推荐算法统一使用英文 snake_case key。                                                                                                                                            |
| 已完成 User Profile Module v0.1 数据库字段设计。确认四张表为 profiles、user_preferences、kitchen_equipment、pantry_items。profiles.id 直接引用 auth.users.id；其他三张私有表使用 user_id uuid references auth.users(id)。确认 user_preferences 的单选字段允许 null，避免把未填写误判为用户明确选择默认值；多选字段使用 text[] 存储标准 key。确认 kitchen_equipment 和 pantry_items 只存用户拥有的 key，不存中文 label、名称、分类或 false 行，相关 label 和 category 后续放在 TypeScript key dictionary 中维护。                                                                                  |
| 已完成 Step 3：types/profile.ts v0.1 设计。确认 types/profile.ts 初版负责 key union types、User Profile 模块 domain types、保存输入类型、前端与推荐算法共用的 key dictionary 常量。确认业务层使用 camelCase domain type，数据库层仍使用 snake_case 字段；snake_case 到 camelCase 的转换后续放在 services 层。确认 types/profile.ts 暂不放 Supabase 查询逻辑、推荐算法、Zustand store、AI prompt 或前端表单状态类型。                                                                                                                                                                              |
| 已完成 Step 4：authService.ts 与 profileService.ts 方法签名和服务边界设计。确认 authService.ts 独立负责 Supabase Anonymous Auth 初始化，提供 ensureAuthUser()、getCurrentUser()、signOut()。确认 profileService.ts 负责 User Profile / Onboarding Foundation 模块业务读写，包含 ensureProfile()、getProfile()、updateProfile()、getUserPreferences()、saveUserPreferences()、getKitchenEquipment()、saveKitchenEquipment()、getPantryItems()、savePantryItems()、getOnboardingContext()。确认 service 层负责 key 校验、数组去重、no_preference 互斥处理、snake_case 与 camelCase 转换和错误抛出。 |
...
确认是否启用 Supabase Anonymous Auth。

若启用，设计首次启动时的 auth.signInAnonymously() 初始化流程。

所有私有业务表统一使用 user_id uuid not null references auth.users(id)。

不设计 guest_id，不允许业务主表长期存在无归属数据。

先设计 profiles、user_preferences、kitchen_equipment、pantry_items 四张表字段。

再设计 ingredients、fridge_scans、fridge_scan_photos、fridge_items。

根据上述表结构反推 profileService、fridgeService、recipeService、recommendationService、aiService 的方法签名。

设计 profiles 表字段，建议 profiles.id = auth.users.id。

设计 user_preferences 表字段和枚举值。

设计 kitchen_equipment 表字段和设备 key 命名规则。

设计 pantry_items 表字段和调料 / 常备食材分类规则。

设计上述四张表的 RLS 策略。

设计 services/authService.ts，提供 ensureAuthUser()。

设计 services/profileService.ts，提供 ensureProfile()、getUserPreferences()、saveUserPreferences()、getKitchenEquipment()、saveKitchenEquipment()、getPantryItems()、savePantryItems()。

字段与 service 边界确认后，生成 Claude Code 执行指令。

设计 user_preferences 第一版 key：portion_size、spice_level、saltiness、cuisine_preferences、meal_style_preferences、diet_tags、disliked_ingredient_keys、allergen_keys。

设计厨房设备第一版 key，控制在 15-25 个以内。

设计 pantry_items 第一版 key 和分类，优先覆盖留学生常见厨房与首批 20-30 道测试菜。

确认菜谱库字段后续需要与这些 key 对齐：cuisine_tags、meal_style_tags、required_equipment_keys、required_pantry_item_keys、spice_level 等。

key dictionary 确认后，进入 User Profile 模块 SQL migration、RLS、types 和 services 设计。

基于 v0.1 key dictionary 设计 profiles 表字段。
基于 v0.1 key dictionary 设计 user_preferences 表字段。
基于 v0.1 key dictionary 设计 kitchen_equipment 表字段。
基于 v0.1 key dictionary 设计 pantry_items 表字段。
为四张表设计唯一约束、索引和 RLS。
后续菜谱库字段必须与本 key dictionary 对齐，包括 cuisine_tags、meal_style_tags、required_equipment_keys、required_pantry_item_keys、spice_level、saltiness 等。

设计 types/profile.ts：定义 profile、preferences、equipment、pantry 相关类型。

设计 key union types：PortionSizeKey、SpiceLevelKey、SaltinessKey、CuisineKey、MealStyleKey、DietTagKey、DislikedIngredientKey、AllergenKey、KitchenEquipmentKey、PantryItemKey。

在 types/profile.ts 或 constants/profileKeys.ts 中维护 key dictionary，包括中文 label、英文 label、category 和推荐算法辅助信息。

后续 SQL migration 需要创建四张表、check constraints、unique constraints、updated_at trigger、RLS policies。

后续 profileService.ts 保存数组字段前需要做 key 校验、数组去重和 no_preference 互斥处理。

后续由 Claude Code 创建 types/profile.ts，包含 key union types、domain types、input types、option dictionary 和 runtime key sets。

等 SQL migration 落地后，再通过 Supabase CLI 生成或更新 types/database.ts。

在 profileService.ts 中实现数据库 snake_case 与业务层 camelCase 的转换。

在 profileService.ts 保存前使用 types/profile.ts 中的 key sets 做输入校验、数组去重和 no_preference 互斥处理。

生成 Claude Code 执行指令，创建 types/profile.ts。
生成 Claude Code 执行指令，创建 services/authService.ts。
生成 Claude Code 执行指令，创建 services/profileService.ts。
生成 Claude Code 执行指令，创建 supabase/migrations/001_user_profile_module.sql。
在 profileService.ts 中实现 key 校验、数组去重和 no_preference 互斥处理。
在 profileService.ts 中实现 snake_case 到 camelCase mapper。
后续评估是否将设备和 pantry 的 delete + insert 保存方式升级为 Postgres RPC 事务函数。
