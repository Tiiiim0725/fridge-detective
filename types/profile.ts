...
export interface ScoredProfileOption<
  T extends string,
> extends ProfileOption<T> {
  score: number;
}
...
export const PANTRY_ITEM_OPTIONS: PantryItemOption[] = [
  ...
  },
  {
    key: "canned_tomato",
    zhLabel: "番茄罐头",
    enLabel: "Canned Tomato",
    category: "canned_frozen_basic",
  },
  {
    key: "frozen_vegetables",
    zhLabel: "冷冻蔬菜",
    enLabel: "Frozen Vegetables",
    category: "canned_frozen_basic",
  },
  {
    key: "frozen_dumplings",
    zhLabel: "冷冻饺子",
...
export const PORTION_SIZE_KEYS: PortionSizeKey[] = PORTION_SIZE_OPTIONS.map(
  (o) => o.key,
);

export const SPICE_LEVEL_KEYS: SpiceLevelKey[] = SPICE_LEVEL_OPTIONS.map(
  (o) => o.key,
);

export const SALTINESS_KEYS: SaltinessKey[] = SALTINESS_OPTIONS.map(
  (o) => o.key,
);

export const CUISINE_KEYS: CuisineKey[] = CUISINE_OPTIONS.map((o) => o.key);

export const MEAL_STYLE_KEYS: MealStyleKey[] = MEAL_STYLE_OPTIONS.map(
  (o) => o.key,
);

export const DIET_TAG_KEYS: DietTagKey[] = DIET_TAG_OPTIONS.map((o) => o.key);

export const DISLIKED_INGREDIENT_KEYS: DislikedIngredientKey[] =
  DISLIKED_INGREDIENT_OPTIONS.map((o) => o.key);

export const ALLERGEN_KEYS: AllergenKey[] = ALLERGEN_OPTIONS.map((o) => o.key);

export const KITCHEN_EQUIPMENT_KEYS: KitchenEquipmentKey[] =
  KITCHEN_EQUIPMENT_OPTIONS.map((o) => o.key);

export const PANTRY_ITEM_KEYS: PantryItemKey[] = PANTRY_ITEM_OPTIONS.map(
  (o) => o.key,
);
...
export function isValidDislikedIngredientKey(
  key: string,
): key is DislikedIngredientKey {
...
export function isValidKitchenEquipmentKey(
  key: string,
): key is KitchenEquipmentKey {
...
export function isValidPantryCategoryKey(
  key: string,
): key is PantryCategoryKey {
